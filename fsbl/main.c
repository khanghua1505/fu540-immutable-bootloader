...
#include "keycore_driver/keycore.h"
...
void secure_boot_main()
{
  keycore_status_t code;
  keycore_info_t info;
  keycore_authentication_t auth;
  
  if (keycore.initfn() != 0) {
    puts("ERROR: Keycore connects failed. Please check connection.\r\n");
    error_handler(__FILE__, __LINE__);
  }
  puts("STATUS: Keycore initializes successfully.\r\n");
  
  puts("\r\nKEYCORE: Get info");
  code = keycore_get_info(&keycore, &info);
  if (code != KEYCORE_OK) {
    puts("\r\nERROR: Keycore connects failed. Please check connection.");
    error_handler(__FILE__, __LINE__);
  }
  puts("\r\nINFO: VERSION "); 
  uart_put_hex((void*)UART0_CTRL_ADDR, info.version);
  puts("\r\nINFO: ID "); 
  uart_put_hex((void*)UART0_CTRL_ADDR, info.id[0]); 
  uart_put_hex((void*)UART0_CTRL_ADDR, info.id[1]);
  
  puts("\r\nKEYCORE: Build...");
  code = keycore_build(&keycore, (void *)DRAM_BASE, sanctum_sm_size);
  if (code != KEYCORE_OK) {
    puts(" FAILED");
    error_handler(__FILE__, __LINE__);
  }
